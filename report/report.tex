...
In order to formalize liquid type checking the simply typed lambda calculus is extended with \lstinline{if}- and \lstinline{let}-expressions plus polymorhism and dependent types.
The typing rules are extended to accommodate those changes.
The liquid type judgement $\Gamma \vdash e: T$ says that the expression $e$ has type $T$ under environment $\Gamma$,
where the environment $\Gamma$ consists of type bindings ($x: T$) and predicates.

In addition there is also a decidable subtype judgment $\Gamma \vdash T_1 <: T_2$ stating that $T_1$ is a subtype of $T_2$ under environment $\Gamma$.
Decidablity is obtained by using a translation into a decidable logic.

Then type safety is obtained using the following theorems, where $\succ$ is the single evaluation step relation:
\begin{enumerate}
    \label{thm:sound}
...
One can show that if this terminating algorithm returns a type 
Also for this terminating algorithm one can show some theorems:
\begin{enumerate}
    \item If \lstinline{Infer}$(\Gamma, e, \mathbb{Q}) = T$ then $\Gamma \vdash e: T$.
    %\item If \lstinline{Infer}($\Gamma, e, \mathbb{Q}$) returns no type then $\nexists T.\, \Gamma \vdash e: T$.
\end{enumerate}
...
To allow array bounds checking the predicates compare ($<, \le, =, \neq, \geq, >$) $\nu$ with 0, program variables and the length of arrays.
In most cases \textbf{DSolve} can prove the safety of array accesses without additional annotations i.e. predicates.
If predicates are missing the inferred types help to identify which ones should be added.
